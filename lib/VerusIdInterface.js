...
const { createUnfundedCurrencyTransfer, createUnfundedIdentityUpdate, validateFundedCurrencyTransfer, completeFundedIdentityUpdate } = utxo_lib_1.smarttxs;
...
class VerusIdInterface {
    ...
    static getExpectedSentFromSweepOutputs(sweepOutputs) {
        const expectedSent = new Map();
        for (const output of sweepOutputs) {
            const satoshis = (0, bignumber_js_1.default)(output.satoshis);
            if (!satoshis.isFinite() || satoshis.isNegative()) {
                throw new Error("Sweep output satoshis must be a non-negative integer string.");
            }
            if (expectedSent.has(output.currency))
                expectedSent.set(output.currency, expectedSent.get(output.currency).plus(satoshis));
            else
                expectedSent.set(output.currency, satoshis);
        }
        return expectedSent;
    }
    static getDestinationFees(destination) {
        let fees = (0, bignumber_js_1.default)(destination.fees != null ? destination.fees.toString() : 0);
        for (const auxDest of destination.auxDests || []) {
            fees = fees.plus(VerusIdInterface.getDestinationFees(auxDest));
        }
        return fees;
    }
    static getExpectedFeesFromSweepOutputs(sweepOutputs, chainId) {
        ...
        for (const output of sweepOutputs) {
            ...
            const outputFees = (0, bignumber_js_1.default)(output.feesatoshis ? output.feesatoshis : "300000")
    ...
    static validateSweepSent(validation, expectedSent) {
        const keys = new Set([
    ...
    static combineUnfundedTransactions(baseTxHex, outputsTxHex) {
        ...
        if (baseTx.ins.length !== 0)
            throw new Error("Identity update transaction must be unfunded before combining sweep outputs.");
        if (outputsTx.ins.length !== 0)
            throw new Error("Currency sweep transaction must be unfunded before combining.");
    ...
    createUpdateIdentityWithCurrencySweepTransaction(identity_1, changeAddress_1, rawIdentityTransaction_1, identityTransactionHeight_1, sweepOutputs_1, utxoList_1) {
        return __awaiter(this, arguments, void 0, function* (identity, changeAddress, rawIdentityTransaction, identityTransactionHeight, sweepOutputs, utxoList, options = {}) {
            if (!sweepOutputs.length)
                throw new Error("Must provide at least one explicit sweep output.");
            ...
            const sweepTxHex = createUnfundedCurrencyTransfer(chainId, sweepOutputs, utxo_lib_1.networks.verus, preparedIdentityUpdate.height + 20);
            const combinedUnfundedTxHex = VerusIdInterface.combineUnfundedTransactions(preparedIdentityUpdate.unfundedTxHex, sweepTxHex);
            let fundedTxHex;
