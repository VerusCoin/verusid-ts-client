...
class VerusIdInterface {
    ...
    createUpdateIdentityTransaction(identity, changeAddress, utxoList, chainIAddr, fee = 0.0001, fundRawTransactionResult, getIdentityResult, identityTransactionResult, currentHeight) {
        return __awaiter(this, void 0, void 0, function* () {
            let height = currentHeight;
            let chainId;
            if (height == null) {
            ...
            const unfundedTxHex = createUnfundedIdentityUpdate(identity, utxo_lib_1.networks.verus, height + 20);
            let fundedTxHex;
            ...
            let idResponse;
            if (getIdentityResult == null) {
                const _idres = yield this.interface.getIdentity(identity.getIdentityAddress());
                if (_idres.error)
                    throw new Error(_idres.error.message);
                idResponse = _idres.result;
            }
            else
                idResponse = getIdentityResult;
            const { txid, vout } = idResponse;
            let rawTxResponse;
            if (identityTransactionResult == null) {
                const rawtxres = yield this.interface.getRawTransaction(txid);
                if (rawtxres.error)
                    throw new Error(rawtxres.error.message);
                rawTxResponse = rawtxres.result;
            }
            else
                rawTxResponse = identityTransactionResult;
            const identityTransaction = utxo_lib_1.Transaction.fromHex(rawTxResponse, utxo_lib_1.networks.verus);
    ...
    createRevokeIdentityTransaction(_identity, changeAddress, utxoList, chainIAddr, fee = 0.0001, fundRawTransactionResult, getIdentityResult, identityTransactionResult, currentHeight) {
        return __awaiter(this, void 0, void 0, function* () {
            const identity = new verus_typescript_primitives_1.Identity();
            identity.fromBuffer(_identity.toBuffer());
            identity.revoke();
            return this.createUpdateIdentityTransaction(identity, changeAddress, utxoList, chainIAddr, fee, fundRawTransactionResult, getIdentityResult, identityTransactionResult, currentHeight);
        });
    }
    /**
     *
